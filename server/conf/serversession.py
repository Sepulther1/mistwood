...
user connecting to the game.  Mistwood manages one Session per
connection to the game. So a user logged into the game with multiple
clients (if Mistwood is configured to allow that) will have multiple
sessions tied to one Account object. All communication between Mistwood
...
This module is not normally called. To tell Mistwood to use the class
...
class ServerSession(BaseServerSession):
    ...
    individual protocols to communicate with Mistwood.
