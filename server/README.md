...
This directory holds files used by and configuring the Mistwood server 
itself.

Out of all the subdirectories in the game directory, Mistwood does
...
When running you will find four new files appear in this directory: 
 ...
 - `server.pid` and `portal.pid`: These hold the process IDs of the
   ...
   Mistwood is shut down uncleanly (e.g. by a crash or via a kill
   signal), these files might erroneously remain behind. If so Mistwood
...
them you can change how Mistwood operates and also plug in your own
...
`settings.py` which is the main configuration file of Mistwood. 

## server/logs/

This subdirectory holds various log files created by the running
Mistwood server. It is also the default location for storing any custom
log files you might want to output using Mistwood's logging mechanisms.
